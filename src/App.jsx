...
import {
  Sun, Shield, Eye, Lock, Sparkles, Phone,
  Calculator, JapaneseYen, Globe
} from "lucide-react";
import { motion } from "framer-motion";
import Hero from "./components/Hero";
import BeforeAfter from "./components/BeforeAfter"; // new slider-supported version
...
const FILMS = [
  {
    ...
    images: [ // <-- add 2–5 real images here
      "/gallery/heat-1.jpg",
      "/gallery/heat-2.jpg",
      "/gallery/heat-3.jpg",
    ],
  ...
  },
  {
    ...
    images: [
      "/gallery/privacy-1.jpg",
      "/gallery/privacy-2.jpg",
    ],
    pricePerM2: 12000,
...
function WindowShieldSite() {
  ...
  const [height, setHeight] = useState(1.0); // meters

  const film = FILMS.find(f => f.key === filmKey) || FILMS[0];
  const area = Math.max(0, Number(width)) * Math.max(0, Number(height));
  const base = film.pricePerM2 * area; // film only
  const total = Number.isFinite(base) ? Math.round(base) : 0; // no misc added

  // Testimonials (language-specific)
  const testimonialsEN = [
    { name: "Sato R.",  text: "Heat-shield film lowered our summer electric bill. Clean install!" },
    { name: "Mika T.",  text: "Privacy film means no more daytime curtains. The room stays bright." },
    { name: "Kenji I.", text: "Security film gives us peace of mind. Family is happy with it." },
  ];
  const testimonialsJP = [
    { name: "佐藤 R.",  text: "遮熱フィルムで夏の電気代が下がりました。施工も丁寧！" },
    { name: "ミカ T.",  text: "目隠しフィルムで日中もカーテン不要に。部屋が明るいままです。" },
    { name: "健司 I.", text: "防犯フィルムで安心感がアップ。家族も喜んでいます。" },
  ];

  /* ---------------- Lightbox state for FILMS grid ---------------- */
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="font-extrabold text-xl tracking-tight">Window Shield Films</div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setLang("en")}
              className={classNames("px-3 py-1 rounded-full text-sm border",
                lang==="en" ? "bg-blue-600 text-white border-blue-600" : "border-neutral-300")}
            >EN</button>
            <button
              onClick={() => setLang("jp")}
              className={classNames("px-3 py-1 rounded-full text-sm border",
                lang==="jp" ? "bg-blue-600 text-white border-blue-600" : "border-neutral-300")}
            >日本語</button>
            <a
      ...
      >
        <h2 className="text-2xl md:text-3xl font-extrabold mb-6 flex items-center gap-2">
          <Globe className="w-6 h-6" />{t.sectionFilms}
        </h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FILMS.map((f, fi) => (
            <article key={f.key} className="rounded-2xl border shadow-sm overflow-hidden bg-white">
              ...
              <div className="p-5">
                ...
                <div className="mt-4 font-bold text-blue-700 flex items-center gap-2">
                  ...
                  <span className="text-neutral-500 font-normal">
                    {" · "}{lang==="jp" ? "諸経費あり（総額はお問い合わせください）" : "misc fee may apply (contact for final total)"}
                  </span>
                </div>

                {/* Thumbnails under card */}
                {f.images?.length > 1 && (
                  <div className="mt-4 flex gap-2 overflow-x-auto">
                    {f.images.slice(0, 4).map((src, ti) => (
                      <button
                        ...
                        title="View larger"
                      >
                        <img src={src} alt={`thumb-${ti}`} className="h-full w-full object-cover" loading="lazy" />
                      </button>
                    ))}
                  </div>
                )}
      ...
      {/* Language gallery — optional, leave as your own */}
      ...
      {/* Before/After slider using two separate images */}
      <section className="max-w-6xl mx-auto px-4 py-12">
        <BeforeAfter
        lang={lang}
        ...
        mode="side"
        showToggle={true}
        sideOrder="before-after"  // Before left, After right (your desired side-by-side)
        sliderBase="after"        // flip slider so After is background, Before is overlay
      />
      ...
      <section id="calculator" className="bg-white border-t border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-12">
          <h2 className="text-2xl md:text-3xl font-extrabold mb-6 flex items-center gap-2">
            <Calculator className="w-6 h-6" />{t.sectionCalc}
          </h2>
          <div className="grid md:grid-cols-2 gap-6 items-start">
            ...
            <div className="rounded-2xl border p-5 bg-neutral-50">
              ...
              <select value={filmKey} onChange={(e)=>setFilmKey(e.target.value)} className="w-full rounded-lg border p-3 bg-white">
                {FILMS.map(f => (<option key={f.key} value={f.key}>{lang==="jp" ? f.jp.title : f.en.title}</option>))}
              </select>

              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  <label className="block text-sm font-semibold mb-1">{t.width}</label>
                  <input type="number" step="0.01" min="0" value={width} onChange={(e)=>setWidth(e.target.value)} className="w-full rounded-lg border p-3" />
                  <p className="text-xs text-neutral-500 mt-1">{t.meters}</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1">{t.height}</label>
                  <input type="number" step="0.01" min="0" value={height} onChange={(e)=>setHeight(e.target.value)} className="w-full rounded-lg border p-3" />
                  <p className="text-xs text-neutral-500 mt-1">{t.meters}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border p-5 bg-white shadow-sm">
              ...
              <div className="mt-2 text-lg font-bold">{lang==="jp" ? film.jp.title : film.en.title}</div>
              <div className="mt-1 text-sm">{t.area}: <span className="font-semibold">{area.toFixed(2)} m²</span></div>
              <div className="mt-1 text-sm">{t.pricePerM2}: <span className="font-semibold">¥{film.pricePerM2.toLocaleString()}</span></div>

              {/* No misc line shown; just a notice */}
              <div className="mt-4 text-3xl font-extrabold text-blue-700">¥{total.toLocaleString()}</div>
              ...
              <a className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-600 text-white px-5 py-3 font-semibold shadow" href={`tel:${PHONE.replaceAll("-", "")}`}>
      ...
      <footer className="bg-blue-700 text-white">
        <div className="max-w-6xl mx-auto px-4 py-10 grid md:grid-cols-2 gap-6 items-center">
          <div>
            <div className="text-2xl md:text-3xl font-extrabold">{t.footerTitle}</div>
            <p className="mt-2 text-blue-100">{t.footerSub}</p>
          </div>
          <div className="md:justify-self-end">
            <a href={`tel:${PHONE.replaceAll("-", "")}`} className="inline-flex items-center gap-3 rounded-2xl bg-white text-blue-700 px-6 py-4 font-extrabold text-xl shadow">
              <Phone className="w-6 h-6" /> {PHONE}
            </a>
            <div className="mt-2 text-sm text-blue-100">{lang==="jp" ? OWNER_JP : OWNER_EN}</div>
          </div>
        </div>
      </footer>

      {/* ------- LIGHTBOX OVERLAY (shared for all films) ------- */}
      {lbOpen && (
        <motion.div
                    ...
                    <div className="mt-3 flex gap-2 overflow-x-auto">
                      {imgs.map((src, i) => (
                        <button
                          ...
                          className={`h-16 w-24 flex-none overflow-hidden rounded-lg border ${i===lbImgIdx ? "ring-2 ring-blue-500 border-blue-500" : "border-white/30"}`}
                        >
